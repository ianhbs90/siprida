...
<div class="container-fluid box">
	<fieldset>
		...
		<form action="<?=$this->config->item('base_url_node');?>reg/submit_reg" id="reg-form" method="POST" class="form-horizontal">
							...
							<div class="input">
								<?php
									if(($path!='5' and $stage!='3') or ($stage=='3' and !is_null($this->session->userdata('nopes')))){
										...
										echo "
										<select name='input_jk' id='input_jk' class='form-control' tabindex='2'>
										<option value=''></option>";
				...
				<div class="col-md-6">
					...
					<div class="form-group">
						...
						<div class="col-md-8">
							<div class="input">
								<?php
									echo "<select name='input_dt2' id='input_dt2' tabindex='7' 
									onchange=\"get_data_linked_toRegency($(this).val(),'".$stage."','".$path."','".$kuota_jalur_row['lintas_dt2']."','".$kuota_jalur_row['jml_sekolah']."');\" class='form-control' ".($stage!='3'?'required':(!$login?'disabled':'required')).">
									<option value=''></option>";
								...
								for($i=1;$i<=$kuota_jalur_row['jml_sekolah'];$i++)
								{
									...
									if($kuota_jalur_row['lintas_dt2']!='0')
									{
										...
										<div id='cont_input_dt2_sekolah_tujuan".$i."'>
											<select name='input_dt2_sekolah_tujuan".$i."' id='input_dt2_sekolah_tujuan".$i."' 
												onchange=\"get_destSchools($(this).val(),'".($path!='5' && $stage!='3'?$peserta_row['dt2_id']:'')."','".$stage."','".$path."','".$kuota_jalur_row['lintas_dt2']."','".$i."');\" 
												class='form-control' ".($stage!='3'?($i==1?'required':''):(!$login?'disabled':'required')).">";
									...
									echo "<td>
										...
										<div id='cont_input_sekolah_tujuan".$i."'>";
											$onchange = ($stage=='1'?"view_school_registrants($(this).val(),'".$path."','".$i."')":"get_destFields($(this).val(),'".$path."','".$i."')");
											echo "
											<select name='input_sekolah_tujuan".$i."' id='input_sekolah_tujuan".$i."' onchange=\"".$onchange."\" class='form-control' ".($stage!='3'?($i==1?'required':''):(!$login?'disabled':'required')).">
												<option value=''>".($path=='3'?'-- Pilih Kab./Kota Domisili lebih dulu --':'-- Pilih Kab./Kota Sekolah lebih dulu --')."</option>";
											echo "</select>
										</div>
										<div id='school-registrants-loader".$i."' style='display:none'>
											<img src='".$this->config->item('img_path')."ajax-loaders/ajax-loader-1.gif'/>
										</div>
										...
										<td>
											...
											<div id='cont_input_kompetensi_tujuan".$i."'>
												<select name='input_kompetensi_tujuan".$i."' id='input_kompetensi_tujuan".$i."' 
														onchange=\"view_field_registrants($(this).val(),'".$path."','".$i."')\"
														class='form-control' ".($i==1?'required':'').">
													<option value=''>- Pilih SMK lebih dulu -</option></select>
											</div>
											<div id='field-registrants-loader".$i."' style='display:none'>
												<img src='".$this->config->item('img_path')."ajax-loaders/ajax-loader-1.gif'/>
											</div>
					...
					<div class='col-md-12'>
						<div class='form-group'>
							...
							<div class='col-md-6'>";
								...
								if($stage!='3')
								{
									echo "<input type='text' class='form-control' name='input_jenis_ujian_smp' value='".$peserta_row['mode_un']."' readonly/>";
								}
								else{
									echo "<select class='form-control' name='input_jenis_ujian_smp'>";
										$opts = array('unbk','unkp');
										foreach($opts as $opt){
											$selected = (strtolower($peserta_row['mode_un'])==$opt?'selected':'');
											echo "<option value='".$opt."'>".strtoupper($opt)."</option>";
										}
									echo "</select>";
								}
							echo "</div>
...
<script type="text/javascript">	
	...
	function get_destSchools(school_regency,regency,school_type,path,accross_regency,i){
		
		ajax_object.reset_object();
		var data_ajax = new Array('school_regency='+school_regency,'regency='+regency,'school_type='+school_type,'path='+path,'accross_regency='+accross_regency,'i='+i);
        ajax_object.set_url($('#baseUrl_node').val()+'reg/get_destSchools').set_data_ajax(data_ajax).set_loading('#dest-school-loader'+i).set_content('#cont_input_sekolah_tujuan'+i).request_ajax();       

        $('#cont_view_school_registrants'+i).html('');

        if(school_type=='2'){
        	$('#input_kompetensi_tujuan'+i).html("<option value''>- Pilih SMK lebih dulu -");
        	$('#cont_view_field_registrants'+i).html('');
        }


	}

	function get_destFields(school,path,i){
		ajax_object.reset_object();
		var data_ajax = new Array('school='+school,'path='+path,'i='+i);
        ajax_object.set_url($('#baseUrl_node').val()+'reg/get_destFields').set_data_ajax(data_ajax).set_loading('#dest-field-loader'+i).set_content('#cont_input_kompetensi_tujuan'+i).request_ajax();
	}

	function get_data_linked_toRegency(regency,school_type,path,accross_regency,n_schools){
		
		$.ajax({
          type:'POST',
          url:$('#baseUrl_node').val()+'reg/get_data_linked_toRegency',
          data:'regency='+regency+'&school_type='+school_type+'&path='+path+'&accross_regency='+accross_regency+'&n_schools='+n_schools,
          beforeSend:function(){    
	...
	function view_school_registrants(school,path,i){
		ajax_object.reset_object();
		var data_ajax = new Array('school='+school,'path='+path);
        ajax_object.set_url($('#baseUrl_node').val()+'reg/view_school_registrants').set_data_ajax(data_ajax)
        		   .set_loading('#school-registrants-loader'+i).set_content('#cont_view_school_registrants'+i).request_ajax();
	}

	function view_field_registrants(field,path,i){
		ajax_object.reset_object();
		var data_ajax = new Array('field='+field,'path='+path);
        ajax_object.set_url($('#baseUrl_node').val()+'reg/view_field_registrants').set_data_ajax(data_ajax)
        		   .set_loading('#field-registrants-loader'+i).set_content('#cont_view_field_registrants'+i).request_ajax();
	}
