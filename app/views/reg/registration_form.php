...
<form action="<?=$this->config->item('base_url_node');?>reg/submit_reg" id="reg-form" method="POST" class="form-horizontal">
	...
	<input type="hidden" id="input_dt2_id" name="input_dt2_id" value="<?=$dt2_id;?>"/>			
	<input type="hidden" id="input_no_kk" name="input_no_kk" value="<?=$no_kk;?>"/>
				...
				<div class="col-md-8">
					<div class="input">
						<?php
							...
							onchange=\"get_data_linked_toRegency($(this).val(),'".$stage."','".$kuota_jalur_row['lintas_dt2']."','".$kuota_jalur_row['jml_sekolah']."');\" 
							class='form-control' required>";
							echo "<option value=''></option>";
							foreach($dt2_rows as $row){
								$selected = ($row['dt2_kd']==$dt2_kd?'selected':'');
								echo "<option value='".$row['dt2_id']."_".$row['dt2_kd']."_".$row['nama_dt2']."' ".$selected.">".$row['nama_dt2']."</option>";
							}
							echo "</select>";
			...
			<div class="form-group">
				...
				<div class="col-md-8">
					...
					<div class="input" id="cont_input_kecamatan">
						<select name="input_kecamatan" id="input_kecamatan" tabindex="8" class="form-control" required>
							<?php
								if($path=='1'){
									foreach($kecamatan_rows as $row){
										$selected = ($row['kecamatan_id']==$district_id?'selected':'');
						...
						for($i=1;$i<=$kuota_jalur_row['jml_sekolah'];$i++)
						{
							...
							if($kuota_jalur_row['lintas_dt2']!='0')
							{
								...
								<div id='cont_input_dt2_sekolah_tujuan".$i."'>
									<select name='input_dt2_sekolah_tujuan".$i."' id='input_dt2_sekolah_tujuan".$i."' onchange=\"get_destSchools($(this).val(),'".($path!='5' && $stage!='3'?$dt2_id:'')."','".$stage."','".$kuota_jalur_row['lintas_dt2']."','".$i."');\" class='form-control' ".($stage!='3'?($i==1?'required':''):(!$login?'disabled':'required')).">";
										echo "<option value=''>".($path!='1'?"-- Pilih Kab./Kota Domisili lebih dulu --":"")."</option>";
