		...
		<tbody>";
			...
			foreach($rows as $row){
					...
					or (($admin_type=='3' or $admin_type=='4') and $this->session->userdata('sekolah_id')==$row['sekolah_id']))
				{
					...
					echo "<td align='right'>".number_format($row['sisa_kuota'])."</td>
					<td align='right'>".number_format($row['jml_tinggal_kelas'])."</td>
					<td align='right'>".number_format($row['grand_kuota'])."</td>
					<td align='center'>";
						if($update_access)
		                	echo "<a href='#' title='Edit' class='btn btn-xs btn-default' id='edit3_".$no."' onclick=\"load_form(this.id)\" data-toggle='modal' data-target='#remoteModal'>";
		                else
		            	...
		            	if($delete_access)
		            		echo "<a href='#' title='Hapus' class='btn btn-xs btn-default' onclick=\"if(confirm('Anda yakin?')){delete_record(this.id,'3')}\" id='delete3_".$no."'>";
		            	else
